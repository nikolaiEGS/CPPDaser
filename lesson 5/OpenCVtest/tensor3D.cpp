...
Tensor3D::Tensor3D(char* image_rec, std::size_t arrSize, std::size_t rowSize, std::size_t colSize) {
	std::size_t startSize = 0;
	std::size_t begin = 0, end = arrSize / 3;
	shape = { 3, rowSize, colSize };

	for (int channel = 0; channel < 3; ++channel) {
		Tensor2D tmp(image_rec, begin, rowSize, colSize);
...
void Tensor3D::printElements() {
	std::for_each(data.begin(), data.end(), [](const Tensor2D& channel) { channel.printElements(); });
}
