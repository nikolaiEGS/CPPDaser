...
public:
	...
	unique_ptr(unique_ptr&& move_ptr) noexcept : unique_ptr(); // works, but problem is arising  becouse of definition outside of the class
	unique_ptr& operator=(unique_ptr&& move_ptr) noexcept;
	constexpr void swap(unique_ptr& left, unique_ptr& right); // should it be friend?
